...
class CustomSaver(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if (epoch % self.save_epochs) == 0:  # or save after some epoch, each k-th epoch etc.
            ...
            for i, block in enumerate(self.model.middle_blocks):
                block.save(self.save_folder +  "/model_{0}_mb{1}.h5".format(epoch, i+1))
...
class VAE(keras.Model):
    def __init__(self, encoder, decoder, middle_blocks, **kwargs):
        ...
        for i, _ in enumerate(middle_blocks):
...
def latent_middle_block(latent_dim):
...
def main():
    ...
    for _ in range(n_middle):
        mb = latent_middle_block(latent_dim)
    ...
    latent_z_folder = base_folder + '/output/vae_hier1/latent_z/{0}_z{1}/run{2}'.format(model_tag, latent_dim, run)
    ...
    vae = VAE(encoder, decoder, middle_blocks)
    ...
    for i, block in enumerate(vae.middle_blocks):
        block.save(model_folder +  "/model_final_mb{0}.h5".format(i+1))
