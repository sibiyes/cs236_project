...
from bigan_modules import fashion_product1
...
class BIGAN(keras.Model):
    ...
    def train_step(self, real_images):
        ...
        with tf.GradientTape() as tape:
            predictions = self.discriminator([combined_images, combined_latent_var])
            d_loss = self.loss_fn(labels, predictions)
        grads = tape.gradient(d_loss, self.discriminator.trainable_weights + self.encoder.trainable_weights, 
...
def main():
    ...
    encoder, decoder, discriminator = fashion_product1(latent_dim)
    model_tag = 'bigan1'
    
    model_folder = base_folder + '/output/bigan1/models/{0}_z{1}/run{2}'.format(model_tag, latent_dim, run)
    image_output_folder = base_folder + '/output/bigan1/image_gen/{0}_z{1}/run{2}'.format(model_tag, latent_dim, run)
